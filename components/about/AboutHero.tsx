...
export default function AboutHero({ dict }: { dict: Dictionary }) {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      gsap.to(".a-parallax-bg", { x: mouseRef.current.x * 20, y: mouseRef.current.y * 20, ease: "power2.out", duration: 1 });
      gsap.to(".a-parallax-fg", { x: mouseRef.current.x * -30, y: mouseRef.current.y * -30, ease: "power2.out", duration: 1 });
    };
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden -z-10 pointer-events-none">
        <div className="a-parallax-bg absolute top-[-10%] left-[-5%] w-[600px] h-[600px] bg-gradient-to-br from-kid-pink/30 to-kid-purple/30 rounded-full blur-[100px] mix-blend-multiply" />
        <div className="a-parallax-bg absolute bottom-[-10%] right-[-5%] w-[600px] h-[600px] bg-gradient-to-tl from-kid-blue/30 to-kid-green/30 rounded-full blur-[100px] mix-blend-multiply" />
        <div className="a-parallax-bg absolute top-[40%] left-[40%] w-[400px] h-[400px] bg-gradient-to-tr from-yellow-300/20 to-orange-300/20 rounded-full blur-[80px] mix-blend-multiply" />
        ...
        <Sparkles className="a-parallax-fg absolute top-32 left-10 md:left-24 w-12 h-12 text-kid-yellow opacity-80 drop-shadow-lg" />
        <Star className="a-parallax-fg absolute bottom-40 left-1/3 w-10 h-10 text-kid-pink opacity-80 drop-shadow-lg" fill="currentColor" />
        <Heart className="a-parallax-fg absolute top-40 right-1/4 w-14 h-14 text-kid-green opacity-80 drop-shadow-lg" fill="currentColor" />
      </div>

      <div className="container mx-auto px-6 md:px-12 relative z-10 text-center max-w-5xl">
        {/* Badge — glassmorphic shimmer */}
        <div className="a-reveal overflow-hidden mb-6">
          <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/60 backdrop-blur-xl border border-white/80 shadow-[0_8px_20px_rgba(0,0,0,0.04)] text-kid-purple font-extrabold text-sm uppercase tracking-wider relative overflow-hidden group hover:shadow-[0_12px_25px_rgba(0,0,0,0.06)] transition-all cursor-default">
            ...
            <span>{dict.about.hero.badge}</span>
          </div>
        </div>

        {/* Title — slate-900 + gradient last words */}
        <div className="overflow-hidden mb-6 pb-2">
          <h1 className="a-reveal text-5xl sm:text-6xl lg:text-[5.5rem] font-black text-slate-900 leading-[1.05] tracking-tight drop-shadow-sm">
            {dict.about.hero.title.trim().split(/\s+/).slice(0, -2).join(' ')}{' '}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-kid-pink via-purple-500 to-kid-blue inline-block pb-2">
              {dict.about.hero.title.trim().split(/\s+/).slice(-2).join(' ')}
            </span>
          </h1>
        </div>
        ...
        <div className="overflow-hidden mb-12">
          <p className="a-reveal text-lg sm:text-xl text-slate-600 font-medium leading-relaxed max-w-2xl mx-auto">
